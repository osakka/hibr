#!/usr/bin/env python3
"""Drive the screen module through a pseudo terminal and read what it emits.
...
Run it directly:  python3 tests/screen.py [path-to-hibr]
"""
...
MOD = os.path.abspath("./build/mods/screen.so")
FAIL = []
...
o, _ = run('screen open\na := screen flush\n'
           'screen put 5 10 "0000"\nb := screen flush\n'
           'screen put 5 10 "0001"\nc := screen flush\n'
           'd := screen flush\nscreen close\necho "R $a $b $c $d"\n')
n = counts(o)
...
o, _ = run('screen open\nscreen flush\nscreen pane box 5 10 3 6\n'
           'screen put -p box 0 0 "abcdefghij"\n'
           'screen put -p box 1 4 "zzzz"\n'
           'screen put -p box 9 0 "offpane"\n'
           'screen flush\nscreen close\n')
d = drawn(o)
...
o, _ = run('screen open\nscreen flush\nscreen pen red blue bold\n'
           'screen put 1 1 "styled"\nscreen flush\nscreen close\n')
...
o, _ = run('screen open\nscreen flush\nscreen key 3000\nscreen close\n'
           'if screen resized; then s := screen size; echo "R $s"; fi\n',
           after=bigger, wait=3)
